...
import { weeklyHours, specialClosures, emergencyLine } from "@/data/hours";
...
const SYSTEM_PROMPT = `You are the friendly and professional AI receptionist for **WhitePeak Dental**, a premium dental clinic in Luxembourg City.

Rules you MUST follow:
1. Be helpful, warm, and concise. Use short paragraphs. Never produce walls of text.
2. When a patient asks about hours, services, pricing, offers, or events — **ALWAYS call the appropriate tool**. Never guess or invent this information.
3. If the patient's request is ambiguous (e.g. "book an appointment" without specifying a date, time, or treatment), **ask a clarifying question first**.
4. For appointment inquiries, collect **date, time, and type of appointment** before calling check_appointment_availability.
5. Prices are returned in EUR cents. Convert to euros for display (e.g. 12000 → €120.00). For price ranges, show "from €X" or "€X – €Y".
6. If asked about insurance coverage, share only information from the data (the coverageNote field). For specific coverage questions, recommend the patient contact their insurance provider.
7. For emergency situations, provide the emergency line number (+352 123 456 789, available 24/7 for registered patients) and advise calling immediately.
8. Stay on-topic. If asked about things unrelated to the dental clinic, politely redirect.
9. The clinic speaks English, French, Luxembourgish, and German. Respond in whatever language the patient uses.
10. Never provide medical diagnoses. If asked about symptoms, recommend booking a consultation.
11. Keep the tone professional yet reassuring — many patients are anxious about dental visits.

Clinic details:
- Address: 24 Avenue de la Liberté, Luxembourg City
- Phone: +352 123 456 789
- Email: hello@whitepeak.lu
- Emergency line: ${emergencyLine.phone} (${emergencyLine.available})`;
...
const tools: ChatCompletionTool[] = [
  {
    ...
    function: {
      name: "get_open_hours",
      description:
        "Get clinic opening hours for a specific date. Returns regular schedule and any special closure / modified hours.",
      parameters: {
        type: "object",
        properties: {
          date: {
  ...
  },
  {
    ...
    function: {
      name: "get_services",
      description:
        "Return the full list of dental services organised by category (general, cosmetic, orthodontics, emergency). Each item includes price, duration, and optional insurance coverage notes.",
      parameters: {
        type: "object",
        properties: {},
        required: [],
  ...
  },
  {
    ...
    function: {
      name: "find_services",
      description:
        "Search or filter dental services by category and/or a free-text query (name or description).",
      parameters: {
        type: "object",
        properties: {
          category: {
            type: "string",
            description: "Filter by category key: general, cosmetic, orthodontics, emergency.",
          },
          query: {
            type: "string",
  ...
  },
  {
    ...
    function: {
      name: "get_events",
      description: "Return upcoming clinic events within an optional date range.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Check appointment availability for a given date, time, and type. Returns available, limited, or unavailable. (Stub — real integration pending.)",
      parameters: {
        type: "object",
        properties: {
          date: { type: "string", description: 'ISO date "YYYY-MM-DD".' },
          time: { type: "string", description: '"HH:mm" (24-hour).' },
          treatment: { type: "string", description: "Type of treatment or appointment." },
        },
        required: ["date", "time"],
      },
    },
...
function toolGetOpenHours(args: { date?: string }): string {
  ...
  const result: Record<string, unknown> = {
    ...
    emergencyLine: `${emergencyLine.phone} (${emergencyLine.available})`,
  };
...
function toolGetServices(): string {
  const compact = services.map((cat) => ({
    ...
    items: cat.items.map((i) => ({
      ...
      coverageNote: i.coverageNote ?? null,
    })),
  }));
...
function toolCheckAppointment(args: {
  ...
  return JSON.stringify({
    date: args.date,
    time: args.time,
    treatment: args.treatment ?? "general consultation",
    status,
    message:
      status === "available"
        ? "Great news — that slot is available! Would you like to book it?"
        : status === "limited"
  ...
  switch (name) {
    ...
    case "check_appointment_availability":
      return toolCheckAppointment(
        args as { date: string; time: string; treatment?: string },
      );
    default:
